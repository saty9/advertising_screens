...
def show_venue(request, venue_id):
    ...
    events = Event.objects.filter(room__venue=venue, end_time__gte=datetime.datetime.now().time())
...
def show_room(request, venue_id, room_id):
    room = get_object_or_404(Room, pk=venue_id)
    events = Event.objects.filter(room=room, end_time__gte=datetime.datetime.now().time())
    current_date = (datetime.datetime.now() - datetime.timedelta(hours=HOUR_BREAK_POINT)).date()
